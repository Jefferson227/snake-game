function setDirection(directionToSet) {
    if (state.direction === getOppositeDirection(directionToSet))
        return;
...
function setNewPosition(snakeHead, directionToSet) {
    let property = directionToSet === Directions.Up || directionToSet === Directions.Down
        ? 'top'
        : 'left';
    
    let currentValue = parseIntFromPixel(snakeHead.css(property));
    let newValue = directionToSet === Directions.Right || directionToSet === Directions.Down
        ? currentValue + BLOCK
        : currentValue - BLOCK
...
function snakeHitsTheWall() {
    ...
    if ((top < 0 || top > edge) || (left < 0 || left > edge))
        return true;
...
function reverseSnakeDirection() {
    ...
    let maxIndex = snakeLength % 2 === 0
        ? snakeLength / 2
        : Math.floor(snakeLength / 2);
...
function snakeHitsItself() {
    ...
    for (let i = 0; i < tailLength; i++) {
        ...
        if (haveSamePosition(snakeHead, tailSegment))
            return true;
    }
...
function restartGame() {
    ...
    $('#game-arena').append(`
        <div id="snake-wrapper" class="snake-wrapper">
            <div id="snake-head" class="snake snake-head" style="top: ${BLOCK}px; left: ${BLOCK * 5}px;"></div>
            <div class="snake snake-tail" style="top: ${BLOCK}px; left: ${BLOCK * 4}px;"></div>
            <div class="snake snake-tail" style="top: ${BLOCK}px; left: ${BLOCK * 3}px;"></div>
            <div class="snake snake-tail" style="top: ${BLOCK}px; left: ${BLOCK * 2}px;"></div>
        </div>
    `);
    $('#game-arena').css({
...
function haveSamePosition(firstElement, secondElement) {
    return firstElement.css('top') === secondElement.css('top')
